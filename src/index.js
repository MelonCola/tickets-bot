...
if (semver.lt(process.versions.node, min_node_version))
	return console.log(
		`\x07Error: Discord Tickets does not work on Node v${process.versions.node}; please upgrade to v${min_node_version} or above.`
	);
...
class Bot extends Client {
	...
	async postStats() {
		 ...
		 */
		if (this.config.super_secret_setting) {
			// you can disable it if you really want
			const data = {
				client: this.user.id,
				guilds: this.guilds.cache.size,
				members: await this.guilds.cache.reduce(
					async (acc, guild) => (await acc) + (await guild.fetch()).approximateMemberCount,
					0
				),
